...
export default async function UFCPage() {
  ...
  const ufcPosts = await prisma.post.findMany({
    where: {
      category: 'UFC', // ✅ Correct casing from Prisma enum
    },
  ...
  return (
    <div className="min-h-screen py-16 px-6 text-white">
      <h1 className="text-4xl font-bold text-yellow-400 mb-4">UFC Picks</h1>
      <p className="text-gray-300 text-lg mb-8">
        View all the latest UFC picks, insights, and strategies shared by FanBetz creators.
      </p>

      {ufcPosts.length === 0 ? (
        <p className="text-gray-500">No UFC picks yet. Check back soon!</p>
      ) : (
