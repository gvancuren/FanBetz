...
export default async function NHLPage() {
  ...
  const nhlPosts = await prisma.post.findMany({
    where: {
      category: 'NHL', // ✅ Prisma enum match
    },
  ...
  return (
    <div className="min-h-screen py-16 px-6 text-white">
      <h1 className="text-4xl font-bold text-yellow-400 mb-4">NHL Picks</h1>
      <p className="text-gray-300 text-lg mb-8">
        View all the latest NHL picks, insights, and strategies shared by FanBetz creators.
      </p>

      {nhlPosts.length === 0 ? (
        <p className="text-gray-500">No NHL picks yet. Check back soon!</p>
      ) : (
