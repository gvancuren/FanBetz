...
export default async function GolfPage() {
  ...
  const golfPosts = await prisma.post.findMany({
    where: {
      category: 'Golf', // ✅ Correct casing for Prisma enum
    },
  ...
  return (
    <div className="min-h-screen py-16 px-6 text-white">
      <h1 className="text-4xl font-bold text-yellow-400 mb-4">Golf Picks</h1>
      <p className="text-gray-300 text-lg mb-8">
        View all the latest Golf picks, insights, and strategies shared by FanBetz creators.
      </p>

      {golfPosts.length === 0 ? (
        <p className="text-gray-500">No Golf picks yet. Check back soon!</p>
      ) : (
