...
export default async function NBAPage() {
  ...
  const nbaPosts = await prisma.post.findMany({
    where: {
      category: 'NBA', // ✅ Must match Prisma enum exactly
    },
  ...
  return (
    <div className="min-h-screen py-16 px-6 text-white">
      <h1 className="text-4xl font-bold text-yellow-400 mb-4">NBA Picks</h1>
      <p className="text-gray-300 text-lg mb-8">
        View all the latest NBA picks, insights, and strategies shared by FanBetz creators.
      </p>

      {nbaPosts.length === 0 ? (
        <p className="text-gray-500">No NBA picks yet. Check back soon!</p>
      ) : (
